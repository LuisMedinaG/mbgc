...
| F-08 | Error envelopes can expose wrapped bad-request/validation messages. ✅ RESOLVED | P2 | S | `services/api/internal/httpx/write.go`, `services/api/internal/apierr/errors.go` | Current policy is safe for unknown errors, but wrapped validation errors may accidentally include internal context. | `WriteError` now uses unwrapped sentinel `Error()` directly; full error (with wrapper text) logged server-side only via `slog.Error("request error", ...)`. |
