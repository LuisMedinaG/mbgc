...
supabase db push
If `supabase db push` fails with auth err, re-link:
...
# Roll back API to the prev image (get tag from prior successful run)
...
| Admin seed not run | Verify `API_SEED_ADMIN_EMAIL` secret is set in GitHub → Settings → Secrets |
