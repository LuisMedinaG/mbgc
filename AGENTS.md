...
- **Pipeline** — `.github/workflows/pipeline.yml`: PR → CI only. Push to dev → CI + deploy API (dev). Push to main → CI + deploy API (prod, manual approval via `production` gate) + deploy web. Go tests run with `-race` and `-coverprofile`; CI fails if coverage drops below 50%.
- **Infra** — `.github/workflows/infra.yml`: `terraform plan` on PR to main, `terraform apply` on merge (infra/ changes only)
...
- **Rotation** — `make rotate-secrets` or `infra/scripts/rotate-secrets.sh`
...
- **Cloudflare** — Pages frontend, DNS for `lumedina.dev`
